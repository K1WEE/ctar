  ...
  template: `
    <div class="bg-white/70 dark:bg-brand-card backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl shadow-xl p-6 flex flex-wrap justify-between items-center gap-4 transition-colors duration-300">
      ...
      <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
        ...
        <button *ngIf="showExport" (click)="onExport.emit()" class="flex-1 md:flex-none px-6 py-2.5 bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-medium rounded-xl transition-all duration-300 shadow-sm transition-colors">
